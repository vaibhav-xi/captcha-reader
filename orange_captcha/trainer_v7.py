...
START_MODEL = "ocr_ctc_infer_safe_v7.keras"
...
def add_occluder(img, strength):

    if np.random.rand() > 0.25 * strength:
        return img

    h,w = img.shape
    x0 = np.random.randint(w//2, w-10)
    cv2.rectangle(img, (x0,0), (w,h), 0, -1)
    return img
...
def gen_train():
    ...
    while True:
        ...
        strength = min(1.0, float(epoch_var.numpy()) / 12.0)
...
class CTCModel(tf.keras.Model):
    ...
    def train_step(self,data):
        ...
        self.optimizer.apply_gradients(
            zip(g,self.trainable_variables))

        gn=tf.linalg.global_norm(
...
def decode_batch(pred):
    ...
    for s in d:
        s=s[s!=-1]
        out.append("".join(
            num_to_char(s+1).numpy().astype(str)))
    return out
...
def run_epoch_test(model,n=120):
    ...
    gt=["".join(num_to_char(real_y[i]+1)
        .numpy().astype(str)) for i in idx]
