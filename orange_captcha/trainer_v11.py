...
DIR_NORMAL   = "../dataset/generated_samples_v8"
DIR_TARGETED = "../dataset/targeted_images_v4"
DIR_REAL     = "../dataset/orange-samples"
DIR_HARD     = "../dataset/hard_negatives_new"

START_MODEL = "ocr_ctc_infer_safe_v9.keras"
...
def load_set(dirpath):
    ...
    for f in os.listdir(dirpath):
        if f.endswith(".png"):
            ...
            labels.append(os.path.splitext(f)[0].split("_")[0])
...
normal_imgs, normal_y, normal_l = load_set(DIR_NORMAL)
target_imgs, target_y, target_l = load_set(DIR_TARGETED)
...
def sample_source():
    ...
    while True:
        k = np.random.choice(len(pools), p=w)
        s = safe_pick(pools[k])
        if s is None:
            continue
        img,y,l = s
...
infer_model.save("ocr_ctc_infer_safe_v11.keras")
print("\nSaved → ocr_ctc_infer_safe_v11.keras")
